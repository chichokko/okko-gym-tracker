...
const ExerciseManager: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
      if (window.confirm("¿Estás seguro de eliminar este ejercicio? Se borrará de las rutinas asociadas.")) {
          const success = await DataService.deleteExercise(id);
          if (success) loadExercises();
      }
  };
    ...
    <div className="space-y-6 animate-in fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Biblioteca de movimientos ({exercises.length})</p>
        </div>
        {!isEditing && (
